...
while not rospy.is_shutdown(): #starts a while loop that runs while rospy is still running
    try:
        (trans,rot) = listener.lookupTransform('/right_elbow_1','/openni_depth_frame',  rospy.Time(0)) #a function that collects the data translation and rotation data published by the /tf node for the coordinate axis fixed to my left (the camera's right) elbow
        ...
        if (euler[1] > foward-tol) and (euler[1] < foward+tol): #checks to see if my body position is in the correct orientation based on the given x-axis position and the tolerance for the pose. Based off of my left elbow (camera's right)
            ...
            teleop.linear.x=1.0 #moves the robot forward at a rate of 1.0 m/s
            print('^^^^ FORWARD ^^^^')
            
        elif (euler2[1] > backward-tol) and (euler2[1] < backward+tol): #Based off of my left hand (camera's right)
        
            teleop.linear.x=-1.0 #moves the robot backward at a rate of 1.0 m/s
            print('vvvv BACKWARD vvvv')
            
        elif (trans1[1] > left-tol1) and (trans1[1] < left+tol1): #Based off of my right elbow (camera's left)
        
            teleop.angular.z=1.0 #turns the robot to the left at a rate of 1.0 rad/s
            print('<<<< TURN LEFT <<<<')
            
        elif (trans3[1] > right-tol1) and (trans3[1] < right+tol1): #Based off of my right hand (camera's left)
            
            teleop.angular.z=-1.0 #turns the robot to the right at a rate of 1.0 rad/s
            print('>>>> TURN RIGHT >>>>')
        ...
        pub.publish(teleop) #publishes the command velocity to the node that is listening on the robot or simulation
